...
```yaml
context_mode: managed    # managed | default
```

**`managed`**: csk assembles a unified agent context file (e.g., `CLAUDE.md`, `agents.md`) from installed skills, ordered by content type priority (4.2). csk owns this file; manual edits are overwritten on `csk install`.

**`default`**: csk installs skills into `.agents/skills/` and creates agent directory symlinks. The developer is responsible for referencing skills in their own context files.
...
| `integrity` | `sha256:` prefixed hex digest of the installed (stripped) file tree. Computed over file contents, independent of git. |
...
**Default mode**: csk creates `.agents/skills/` and agent-specific symlinks. The developer manually references skills in their own context files (CLAUDE.md, agents.md, etc.).

**Managed mode**: csk generates and owns the agent's context file. The generated file contains:

1. A header comment: `# Generated by CocoaSkill -- DO NOT EDIT`.
2. Sections assembled from installed skills, ordered by content type priority (4.2).
3. Injection protection context (9.9).

In managed mode, csk overwrites the context file on every `csk install`. Manual edits to the generated file are lost.

### 8.3 Context Assembly (Managed Mode)

Assembly order within the generated context file:

1. **Injection protection header** (9.9) -- always first.
2. **Persona** content -- skills with `type: persona`.
3. **Context** content -- skills with `type: context`.
4. **Skill** content -- skills with `type: skill`.
5. **Toolchain** content -- skills with `type: toolchain` are referenced by path, not inlined.
